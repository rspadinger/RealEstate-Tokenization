...
//@note contract adds support cross-chain transfers via Chainlink CCIP. =>
//any cross-chain NFT implementation requires at least two smart contracts on two blockchains and interconnection between them
//We are going to implement the Burn-and-Mint mechanism
//we will fund the CrossChainBurnAndMintERC1155.sol smart contract with LINK tokens for CCIP fees, the Withdraw.sol smart contract will
//hold a logic for withdrawal any ERC-20 token and native coins from this smart contract.
contract CrossChainBurnAndMintERC1155 is ERC1155Core, IAny2EVMMessageReceiver, ReentrancyGuard, Withdraw {
  ...
  function crossChainTransferFrom(
    ...
    Client.EVM2AnyMessage memory message = Client.EVM2AnyMessage({
      receiver: abi.encode(s_chains[destinationChainSelector].xNftAddress), //encode dest token addr
    ...
    if (payFeesIn == PayFeesIn.LINK) {
      ...
      // Send the message through the router and store the returned message ID - pay fees in ETH
    ...
    onlyEnabledSender(message.sourceChainSelector, abi.decode(message.sender, (address)))
  {
    ...
    (address from, address to, uint256 id, uint256 amount, bytes memory data, string memory tokenUri) = abi.decode(
      message.data,
