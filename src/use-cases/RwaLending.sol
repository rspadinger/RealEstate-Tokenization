...
contract RwaLending is IERC1155Receiver, OwnerIsCreator, ReentrancyGuard {
  ...
  uint256 internal immutable i_ltvInitialThreshold; //60%
  uint256 internal immutable i_ltvLiquidationThreshold; //75%
  ...
  constructor(
    ...
    i_ltvInitialThreshold = 60; //borrow upt to 60 USDC for $100 real estate value
    i_ltvLiquidationThreshold = 75; //if RE value drops below 75 => liquidation
  }
  ...
  function liquidate(uint256 tokenId, address borrower) external {
    ...
    uint256 normalizedValuation = (getValuationInUsdc(tokenId) * loanDetails.erc1155AmountSupplied) /
  ...
  function getValuationInUsdc(uint256 tokenId) public view returns (uint256) {
    ...
    uint256 valuation = (i_weightListPrice *
      ...
      priceDetails.taxAssessedValue) / (i_weightListPrice + i_weightOriginalListPrice + i_weightTaxAssessedValue);

    uint256 usdcPriceInUsd = getUsdcPriceInUsd();

    uint256 feedDecimals = s_usdcUsdAggregator.decimals(); //8 for USDC
    uint256 usdcDecimals = 6; // USDC uses 6 decimals => 1 USDC == 1000000
  ...
  //To be able to receive ERC-1155 tokens, we must implement the IERC1155Receiver => onERC1155Received => make sure, sender is our RealEstate token
